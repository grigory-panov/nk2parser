...
namespace OutlookAddInTest
{
    public partial class ThisAddIn
    {
        ...
        private void ThisAddIn_Startup(object sender, System.EventArgs e)
        {
            ...
            byte[] content = propertyAcc.GetProperty(PR_ROAMING_BINARYSTREAM);// );
                    ...
                    case PROPERTY_VALUE_DATA:
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else if (currentTag == PR_ENTRYID)
                                {
                                    if(!string.IsNullOrEmpty(email)){
                                        ContactItem result = (ContactItem) inboxFolder.Items.Find("[email]='"+email+"'");
                                        if (result != null)
                                        {
                                            textToReplace = result.FullName;
                                            Console.WriteLine("replace to " + textToReplace);
                                        }
                                        else
                                        {
                                ...
                                propertiesCount--;
                                Console.WriteLine("END OF PROPERTY, LEFT:" + propertiesCount);
                                idx = 0;
                                ...
                                else
                                {
                                    ...
                                    email = null;
                                    rowsCount--;
                                    Console.WriteLine("END OF ROW, LEFT:" + rowsCount);
                                    if (rowsCount > 0)
                                    {
                                        state = NUMBER_OF_PROPERTIES;
                                        buffer = new byte[4];
            ...
            propertyAcc.SetProperty(PR_ROAMING_BINARYSTREAM, a);
            storage.Save();
        ...
        private void printTag(byte[] buffer)
        {
            ...
            {
                Console.WriteLine(">>>>>>>>>>>>>>>>>>>PR_DISPLAY_NAME_W, Hex: {0:x}", value);
            }
            if (value == PR_DROPDOWN_DISPLAY_NAME_W)
            {
                Console.WriteLine(">>>>>>>>>>>>>>>>>>>PR_DROPDOWN_DISPLAY_NAME_W, Hex: {0:x}", value);
            }
            else if (value == PR_ENTRYID)
            {
                Console.WriteLine(">>>>>>>>>>>>>>>>>>PR_ENTRYID, Hex: {0:x}", value);
            }
            else
            {
                Console.WriteLine("Not important tag, Hex: {0:x}", value);
            }
        }
